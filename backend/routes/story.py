...
@router.post("/story", response_model=story_schema.Story)
async def generate_story(db: Session = Depends(auth.get_db), current_user: User = Depends(auth.get_current_user)):
