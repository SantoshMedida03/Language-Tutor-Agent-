...
class GrammarQuizNode:
    """A node that generates a grammar quiz independent of a story."""
    def __init__(self):
        self.llm = ChatGoogleGenerativeAI(model="models/gemini-2.5-flash", google_api_key=os.getenv("GEMINI_API_KEY"))
        self.prompt_template = PromptTemplate.from_template(
            """
            You are an English grammar expert. Generate a 5-question multiple-choice quiz
            on a random, common English grammar topic (e.g., verb tenses, prepositions, articles, plurals).
            ...
            - "choices": A list of 4 strings representing the possible answers.
            - "correct_answer": The string from the "choices" list that is the correct answer.
            ...
            {{
              "questions": [
                {{
                  ...
                  "choices": ["He go to the store.", "He goes to the store.", "He goed to the store.", "He going to the store."],
                  "correct_answer": "He goes to the store."
                }}
              ]
            }}
            """
    ...
    def __call__(self, state: ChatState) -> ChatState:
        ...
        try:
            # Add logging to see the raw output from the LLM
            print("--- RAW LLM OUTPUT FOR GRAMMAR QUIZ ---")
            print(quiz_str)
            print("------------------------------------")
            ...
            questions_for_db = []
            answers_for_db = []
            # Correctly check if the 'questions' key exists and is a non-empty list
            ...
            state.tutor_response = "I've just created a new grammar quiz for you! You can find it in the Quiz section of your dashboard."
            
        except (json.JSONDecodeError, TypeError):
            state.quiz = {}
            state.tutor_response = "I had a little trouble creating a grammar quiz. Let's try again!"
